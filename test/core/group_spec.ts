...
describe('Groups', () => {
  ...
  it('create a new group', async () => {
    ...
    expect(await groupCore.groupExists(NEW_GROUP_NAME)).to.containSubset({
      name: NEW_GROUP_NAME,
    });
  });
  ...
  it('create saldo for user', async () => {
    const user = await groupCore.addUserToGroup(
      USER_NOT_IN_GROUP.username,
      GROUP.groupName,
    );
  ...
  it("will set user's default group to new group if user has no other groups", async () => {
    ...
    await groupCore.addUserToGroup(
      USER_NOT_IN_ANY_GROUP.username,
      GROUP.groupName,
    );
