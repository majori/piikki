...
describe('Global API', () => {
  ...
  it('create a new user', async () => {
    ...
    const res1 = await API.post('/users/create', {
      username,
      password: 'hackme',
    });
  ...
  it('delete user', async () => {
    const res1 = await API.del('/users', { username: USER.username });

    helper.expectOk(res1);

    // Check that user can not be found anymore
    helper.expectError(await API.get(`/users/${USER.username}`), 404);

    // The user is cannot be found in the group
    const res2 = await API.get(`/groups/${GROUP.groupName}/members`);
    expect(_(res2.body.result).map('username').includes(USER.username)).to.be
      .false;
  ...
  it('reset username', async () => {
    ...
    helper.expectOk(
      await API.get(`/groups/${GROUP.groupName}/members/${newUsername}`),
    );
  ...
  it('get group', async () => {
    ...
    expect(res1.body.result.members).to.have.length(
      seed.meta.membersInGroup.group1,
    );
    ...
    helper.expectError(await API.get('/groups/-1'), 400);

    helper.expectError(await API.get('/groups/NOT_FOUND'), 404);
  });
  ...
  it('get group member', async () => {
    const res = await API.get(
      `/groups/${GROUP.groupName}/members/${USER.username}`,
    );
    helper.expectOk(res);
    expect(res.body.result).to.have.property('username', USER.username);
  ...
  it('create a group', async () => {
    ...
    // Create new group
    const res1 = await API.post('/groups/create', { groupName: newGroup });
  ...
  it('create a private group', async () => {
    ...
    const res1 = await API.post('/groups/create', {
  ...
  it('add member to private group', async () => {
    ...
    const res1 = await API.post(`/groups/${group}/addMember`, {
      username: USER.username,
      password,
    });
    helper.expectOk(res1);
    ...
    helper.expectError(
      await API.post(`/groups/${group}/addMember`, {
        username: seed.data.users[1].username,
        password: 'wrong',
      }),
      400,
    );
